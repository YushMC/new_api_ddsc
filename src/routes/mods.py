...
def list_mods_admin_all(
    db: Session = Depends(db_init.get_db),
    user: TokenUser = Depends(verify_admin_role)
):
    """
    Listar TODOS los mods sin limitaciones de paginación (solo para OWNER/EDITOR)
    
    ⚠️ Este endpoint retorna TODOS los mods incluyendo inactivos, SIN paginación.
    Excluyendo los que requieren revisión.
    ...
    return {
        "response": "success",
        "message": f"Se obtuvieron {len(prepared_mods)} mods totales sin limitaciones",
        "data": prepared_mods
    }

@router.get("/admin/revision-unpaginated")
def list_mods_pending_revision_all(
    db: Session = Depends(db_init.get_db),
    user: TokenUser = Depends(verify_admin_role)
):
    """
    Listar TODOS los mods que requieren revisión sin limitaciones de paginación (solo para OWNER/EDITOR)
    
    ⚠️ Este endpoint retorna TODOS los mods en revisión, SIN paginación.
    ...
    return {
        "response": "success",
        "message": f"Se obtuvieron {len(prepared_mods)} mods en revisión sin limitaciones",
        "data": prepared_mods
    }
